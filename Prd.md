...
| 版本 | 日期 | 更新内容 | 状态 |
| :--- | :--- | :--- | :--- |
| **V1.4** | 2026-03-15 | **今日计划体验优化**。<br>1. **智能排序**：今日计划默认按计划时间自动排序（早→晚），无时间的任务排在末尾；支持拖拽手动调整顺序（优先级更高）。<br>2. **关联自动切换**：选择关联蓄水池任务时，自动切换属性为对应分类（用户仍可手动覆盖）。<br>3. **删除二次确认**：删除今日计划时弹出简洁确认弹窗，防止误删。<br>4. **标题时间解析**：输入任务时若标题包含时间段（如”15:00-16:00 开发”），自动解析并填充计划时间，标题自动清理。 | **Current** |
| V1.3 | 2026-03-11 | **交互优化与AI升级**。<br>1. **时间控件重构**：支持直接数字输入（HH:MM）+ 自定义简约时间选择器（替代原生picker）；智能跳转（输入2位后自动跳到分钟）；智能补零（输入>2自动补0）。<br>2. **今日计划拖拽排序**：支持拖拽调整今日任务优先级顺序。<br>3. **关联功能完善**：实际时间记录新增”关联蓄水池”下拉框。<br>4. **AI洞察精简**：输出更简短（3段），深度洞察整合为1段，语气更友好温暖。<br>5. **错误提示优化**：飞书权限错误(91403)给出明确的中文操作指引。 | 已发布 |
