...
| 版本 | 日期 | 更新内容 | 状态 |
| :--- | :--- | :--- | :--- |
| **V1.4.1** | 2026-03-15 | **今日计划体验优化**。<br>1. **智能排序**：新增任务后自动按计划时间排序（早→晚），无时间的任务排在末尾；支持拖拽手动调整顺序。<br>2. **关联自动切换**：选择关联蓄水池任务时，自动切换属性为对应分类（用户仍可手动覆盖）。<br>3. **删除二次确认**：删除今日计划时弹出简洁确认弹窗，防止误删。<br>4. **标题时间解析**：输入任务时若标题包含时间段（如”15:00-16:00 开发”），自动解析并填充计划时间（保留完整标题）。 | **Current** |
